(function ($) {
    ...
    //----------------------------------------------------------------------------------------
    //在plugin.js插件中niceSelect()方法与angularjs的级联菜单有冲突,所以在此处将该方法的调用注释掉
    //如果需要让html页面中的select标签具有更nice的效果,可以导入plugin.css样式表,并给select标签添加一个属性class="nice-select"即可
    //----------------------------------------------------------------------------------------
    // $('select').niceSelect();
    ...
    //给每个一级类型的标签绑定一个click事件
    // $(".firstLevelCategory").on("click",function(){
    //     alert($(this).html());
    //     // $(this).next("ul").slideDown(1000).parent().siblings("li").find("ul").slideUp(1000);
    // });
